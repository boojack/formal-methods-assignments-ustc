...
# Try to find the whole expression
def get_the_whole_expr(memory, expr: ExprBop):
    left = copy.deepcopy(expr.left)
    right = copy.deepcopy(expr.right)
    if isinstance(left, ExprVar):
        left = memory.symbolic_memory[left.var]
    if isinstance(right, ExprVar):
        right = memory.symbolic_memory[right.var]
...
def concolic_stmt(memory, stmt):
    if isinstance(stmt, StmtAssign):
        ...
        if interpret_expr(memory, stmt.expr):
            ...
            memory.path_condition.append(stmt.expr)
        else:
        ...
        # Save the last loop judgment expr that not fit while expression
        if isinstance(stmt.expr, ExprBop):
            memory.path_condition.append(get_the_whole_expr(memory, stmt.expr))
...
def concolic_executor(func, init_params, try_times):
    ...
    for try_time in range(2, try_times+1, 1):
        ...
        # random_idx = len(memory.path_condition) - 1
...
if __name__ == '__main__':
    ...
    NOTE:
    由于这里是随机取反，所以存在如下情况：
    无论 try_times 取多大，可能存在某种"路径"没走到。
    '''
