    ...
    def compile_expr(expr):
        ...
        if isinstance(expr, calc.ExprBop):
            ...
            if expr.bop is calc.BOp.ADD:
                tac_stmts.append(tac.StmtAssignAdd(new_var, left, right))
            elif expr.bop is calc.BOp.SUB:
                tac_stmts.append(tac.StmtAssignSub(new_var, left, right))
            elif expr.bop is calc.BOp.MUL:
                tac_stmts.append(tac.StmtAssignMul(new_var, left, right))
            elif expr.bop is calc.BOp.DIV:
                tac_stmts.append(tac.StmtAssignDiv(new_var, left, right))
            return new_var
...
def translation_validation(original_func: calc.Function, result_func: tac.Function):
    ...
    F = Implies(z3.And(original_cons + result_cons), original_cons[-1].arg(0)==result_cons[-1].arg(0))
